...
<div class="progress mt-2" style="height: 50px;">
    <div class="progress-bar bg-red progress-bar-striped progress-bar-animated " role="progressbar"
        ...
        <h1 class="h1">قرعه کشی عالیان</h1>
    </div>
</div>
...
<div class="d-flex justify-content-center">
    @php
        $allChances=0;
        foreach ($users as  $user){
	    $allChances=$allChances+$user->number_of_chance;
 }
    @endphp
    <h3>تعداد قرعه کشی باقی مانده :       {{$allChances}}  </h3>
</div>
@if ($errors->any())
...
<hr>
    ...
    <div class="container">
        ...
        <form method="post" action="/lottery">
            @csrf
            <button  class="btn btn-success w-100 btn-lg  my-5 ">                 شروع قرعه کشی
            </button>
        </form>
@isset($winnerName)
        ...
        <div class="d-flex my-5 justify-content-center">
            <h1 class="mx-5">برنده این دوره قرعه کشی</h1>
        </div>

    <div class="container border border-warning shadow-lg p-3 mb-5 bg-white rounded d-flex justify-content-center">
        <h1 class="h2"> تبریک به {{$winnerName}} </h1>
    </div>
